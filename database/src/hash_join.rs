...
fn partition_input<R: Read, W: Write>(
    ...
    // Use an 8MB shared budget across all partitions.  Larger flush batches
    // mean fewer disk calls (one write_blocks_batch call per flush, covering
    // all blocks in that flush regardless of count).
    const PARTITION_BUFFER_BUDGET: usize = 8 * 1024 * 1024;
    let rows_per_flush = (PARTITION_BUFFER_BUDGET / num_partitions / row_size_bytes.max(1)).max(32);
    ...
    while let Some(row) = input.next(buffer_pool) {
        ...
        if buckets[bucket_id].len() >= rows_per_flush {
            ...
            block_ids[bucket_id].extend(start_block..start_block + num_blocks);
