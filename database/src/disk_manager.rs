...
impl<R: Read, W: Write> DiskManager<R, W> {
    ...
    pub fn read_blocks(&mut self, start_id: u64, count: u64) -> Result<Vec<u8>> {
        ...
        // Skip the implicit zero-init that `vec![0u8; total_bytes]` does — read_exact
        // fully fills the buffer or returns Err (in which case we propagate and the
        // caller never observes the contents). On a 4 MB sequential-scan batch this
        // saves a full memset per read.
        let mut buf: Vec<u8> = Vec::with_capacity(total_bytes);
        // SAFETY: capacity == total_bytes and read_exact below initializes the
        // entire range before any caller reads it. If read_exact errors, `buf` is
        // dropped without any read happening.
        unsafe {
