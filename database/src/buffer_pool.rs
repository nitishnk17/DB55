...
impl<R: Read, W: Write> BufferPool<R, W> {
    ...
    /// Write multiple consecutive blocks to disk in a single disk call (bypasses LRU cache).
    /// `data` must be exactly `num_blocks * block_size` bytes.
    /// Use this to batch-write all blocks of a run in one I/O call, reducing
    /// total_writes from O(num_blocks) to O(1) per run/flush.
    pub fn write_blocks_batch(&mut self, start_block_id: u64, data: &[u8]) {
        self.disk_manager.write_blocks(start_block_id, data).unwrap();
    }
